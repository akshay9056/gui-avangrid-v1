...
resource "kubernetes_ingress_v1" "backend_ingress" {
  ...
  spec {
    ...
    tls {
      hosts       = ["backend.avangrid.com"] #replace this with domain they would serve for frontend
      secret_name = "backend-tls"
    }

    rule {
      host = "backend.avangrid.com" #replace this with domain they would serve for frontend
      http {
        path {
          path      = "/"
          path_type = "Prefix"
